...
/// </summary>
public class Nrf24l01_2 : IDisposable
{
    ...
    public Nrf24l01_2(SpiDevice sensor, int ce, int irq, byte packetSize, byte channel = 2,
        OutputPower outputPower = OutputPower.N00dBm, DataRate_ dataRate = DataRate_.Rate2Mbps, PinNumberingScheme pinNumberingScheme = PinNumberingScheme.Logical, GpioController? gpioController = null, bool shouldDispose = true)
    {
    ...
    public Span<byte> Receive(byte length)
    {
        ...
        Write(Command.NRF_W_REGISTER, Register.NRF_STATUS, 1 << 6);
        _gpio.Write(_ce, PinValue.High);
...
#endif
    private void Initialize(PinNumberingScheme pinNumberingScheme, OutputPower outputPower, DataRate_ dataRate, byte channel, GpioController? gpioController)
    {
        // open pins
    ...
    {
        return WriteCmdOnly(Command.NRF_NOP);
    }
    ...
    internal void SetDataRate(DataRate_ rate)
    {
        // Details in the Datasheet P54
        // Register: 0x06, bit[3]
        _gpio.Write(_ce, PinValue.Low);
    ...
    internal DataRate_ ReadDataRate()
    {
        // Details in the Datasheet P54
        Span<byte> readData = WriteRead(Command.NRF_R_REGISTER, Register.NRF_RF_SETUP, 1);

        return (DataRate_)((readData[0] & 0b_0000_1000) >> 3);
    }
    ...
    internal byte WriteCmdOnly(Command command)
    {
        Span<byte> writeBuf = stackalloc byte[1]
        {
            (byte)((byte)0x20 | (byte)command)
        };
...
public class Nrf24l01Pipe
{
    ...
    private readonly Nrf24l01_2 _nrf;
    ...
    public Nrf24l01Pipe(Nrf24l01_2 nrf, byte pipeID)
    {
        _nrf = nrf;
        _pipeID = pipeID;
    }
